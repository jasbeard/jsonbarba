...
>(({ ...props }, ref) => {
  ...
  return (
    <div ref={ref} {...props}>
      ...
      {workListContext?.highlights &&
        workListContext?.highlights.map((exp, index) => (
          <ul
            key={index}
            className="list-disc list-inside marker:text-[#d1d5db] dark:marker:text-[#4b5563]"
          >
            <li>{exp}</li>
          </ul>
        ))}
...
>(({ ...props }, ref) => {
  ...
  return (
    <div ref={ref} {...props}>
      {!!workListContext?.technology ? "Technology: " : null}{" "}
      {workListContext?.technology?.join(", ")}
    </div>
  );
});
